...
## 变更 16：修复设备状态不会自动恢复为活跃的问题

- **变更的日期**：2026-03-17
- **变更的文件**：app/services/latency_service.py
- **变更的位置**：_update_device_latency 方法（第145-148行）
- **变更的内容**：
  1. 在检测成功时，添加状态恢复逻辑：如果设备当前状态是 "offline"，则将其恢复为 "active"
  2. 调整条件判断结构，将成功和失败的逻辑分开处理
- **变更的原因**：
  - **问题现象**：设备之前SSH连接失败被标记为"离线"，但恢复连接后状态不会自动变回"活跃"
  - **根因分析**：在 latency_service.py 的 _update_device_latency 方法中，只有检测失败时才会更新状态为 "offline"，但检测成功时没有代码将状态从 "offline" 恢复为 "active"
