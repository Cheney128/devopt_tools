...
**AsyncIOScheduler vs BackgroundScheduler** - Phase1 更新：

| 调度器类型 | 使用场景 | 特点 |
|------------|----------|------|
| AsyncIOScheduler | FastAPI 应用 | 支持 async 任务，与事件循环兼容 |
| BackgroundScheduler | 独立进程/线程 | 后台运行，不依赖事件循环 |

**Phase1 迁移**：
- `arp_mac_scheduler`: 从 BackgroundScheduler 迁移到 AsyncIOScheduler
- `ip_location_scheduler`: 使用 AsyncIOScheduler
- `backup_scheduler`: 使用 AsyncIOScheduler

**Session 生命周期问题解决**：
```python
# arp_mac_scheduler.py - 任务内部重新获取 Session
async def _run_collection_async(self):
    db = SessionLocal()  # 任务内部获取 Session
    try:
        stats = await self.collect_all_devices_async(db)
        # ...
    finally:
...
class IPLocationCalculator:
    def _load_arp_entries(self) -> Dict[str, ARPEntry]:
        """批量加载 ARP 表项到内存"""
        entries = self.db.query(ARPEntry).all()
        return {e.ip_address: e for e in entries}

    def _load_mac_entries(self) -> Dict[str, List[MACEntry]]:
        """批量加载 MAC 表项到内存"""
        entries = self.db.query(MACAddressCurrent).all()
        return self._group_by_mac(entries)
    ...
    def calculate_batch(self) -> dict:
        ...
        """
        # 复用加载的数据避免重复查询
        self._arp_entries = self._load_arp_entries()
        self._mac_entries = self._load_mac_entries()
        # ... 匹配计算
```
...
```python
# 归档条件
def should_archive(ip_location: IPLocationCurrent) -> bool:
    """
    两级验证：
    1. calculated_at 超过阈值时间（默认 30 分钟）
    2. IP 不在当前 ARP 表中
    """
    threshold_time = datetime.now() - timedelta(minutes=offline_threshold)

    # 第一级：时间阈值
    if ip_location.calculated_at >= threshold_time:
        return False

    # 第二级：ARP 存在性检查
    if ip_location.ip_address in current_arp_ips:
        return False

    return True
```

### 7.6 ARP/MAC 采集功能相关 (新增 Phase1 2026-03-31)

| 技术 | 用途 | 说明 |
|------|------|------|
| AsyncIOScheduler | 定时调度 | 支持 async 任务，与 FastAPI 事件循环兼容 |
| asyncio.gather() | 并行采集 | 同时采集 ARP 和 MAC 表 |
| asyncio.to_thread() | 同步包装 | 包装同步数据库操作（commit/rollback） |
| UPSERT 策略 | 数据写入 | MySQL INSERT ... ON DUPLICATE KEY UPDATE |
| 懒初始化模式 | 连接池 | SSHConnectionPool 延迟创建 asyncio 对象 |
| FastAPI lifespan | 生命周期 | 统一管理所有 scheduler 的启动和关闭 |
...
class ARPMACScheduler:
    def __init__(self, interval_minutes: int = 30):
        self.scheduler = AsyncIOScheduler()  # 支持 async 任务
    ...
    async def _run_collection_async(self):
        ...
        try:
            ...
            await asyncio.to_thread(calculator.calculate_batch)
        finally:
            db.close()  # 任务完成后关闭 Session
```

**懒初始化模式（SSHConnectionPool）**：
...
- APScheduler实现定时任务调度
- PyJWT + Passlib实现安全认证
- IPLocationCalculator实现 IP 定位预计算（新增）
...
整体技术栈成熟稳定，社区活跃，文档完善，适合企业级应用开发。新增的用户认证、批量备份、监控面板、IP 定位等功能，进一步完善了系统的功能体系，提升了系统的安全性、可维护性和网络管理能力。
