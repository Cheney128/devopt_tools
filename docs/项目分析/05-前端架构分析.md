...
│  │  └──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────┘  │   │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐               │   │
│  │  │ 配置管理 │ │ 设备采集 │ │ Git配置  │ │ IP定位   │ (新增)        │   │
│  │  │  Config  │ │ Collection│ │  Git     │ │IPLocation│               │   │
│  │  └──────────┘ └──────────┘ └──────────┘ └──────────┘               │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                    │                                        │
│                                    ▼                                        │
...
│   │   ├── BackupMonitoring.vue      # 备份监控 (新增)
│   │   ├── BackupScheduleManagement.vue # 备份计划管理 (新增)
│   │   └── ip-location/              # IP 定位模块 (新增)
│   │       ├── IPLocationIndex.vue   # IP 定位主页
│   │       ├── IPLocationSearch.vue  # IP 搜索页面
│   │       └── IPLocationList.vue    # IP 列表页面
│   │
│   ├── App.vue                 # 根组件
...
| /login | login | LoginView | 登录页面 (新增) |
| /users | users | UserManagement | 用户管理页面 (新增) |
| /profile | profile | ProfileView | 个人中心页面 (新增) |
| /backup-monitoring | backup-monitoring | BackupMonitoring | 备份监控页面 (新增) |
| /ip-location | ip-location | IPLocationIndex | IP 定位主页 (新增) |
| /ip-location/search | ip-location-search | IPLocationSearch | IP 搜索页面 (新增) |
| /ip-location/list | ip-location-list | IPLocationList | IP 列表页面 (新增) |
...
| 登录 | LoginView.vue | 用户登录、验证码 (新增) |
| 用户管理 | UserManagement.vue | 用户CRUD、角色管理 (新增) |
| 个人中心 | ProfileView.vue | 个人信息修改 (新增) |
| 备份监控 | BackupMonitoring.vue | 备份统计、趋势图表 (新增) |
| 备份计划管理 | BackupScheduleManagement.vue | 备份计划配置 (新增) |
| IP 定位主页 | ip-location/IPLocationIndex.vue | IP 定位入口页面 (新增) |
| IP 搜索 | ip-location/IPLocationSearch.vue | IP 地址定位搜索 (新增) |
| IP 列表 | ip-location/IPLocationList.vue | IP 定位结果列表 (新增) |
...
## 7. IP 定位前端模块 (新增 2026-03-27)

### 7.1 模块结构

```
frontend/src/views/ip-location/
├── IPLocationIndex.vue    # 主入口页面（Tab 导航）
├── IPLocationSearch.vue   # IP 地址搜索页面
└── IPLocationList.vue     # IP 列表页面
...
export const ipLocationApi = {
  // 搜索 IP 定位
  searchIP: (ipAddress) => apiClient.get(`/ip-location/search/${ipAddress}`),

  // 获取 IP 列表
  getIPList: (params) => apiClient.get('/ip-location/list', { params }),

  // 获取收集状态
  getCollectionStatus: () => apiClient.get('/ip-location/collection/status'),

  // 触发数据收集
  triggerCollection: () => apiClient.post('/ip-location/collection/trigger')
}
```
...
| 页面 | 功能 |
|------|------|
| IPLocationIndex | 主入口，Tab 导航，收集状态显示，触发收集按钮 |
| IPLocationSearch | IP 地址搜索，显示定位结果（设备、接口、置信度） |
| IPLocationList | IP 列表分页展示，搜索过滤，导出功能 |
...
7. **IP 定位功能**：完整的 IP 定位前端模块，支持搜索、列表、收集状态展示（新增）
