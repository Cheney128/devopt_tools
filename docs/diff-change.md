...
pandas==2.2.0
```
**变更原因**：修复生产环境批量导入设备时后端服务崩溃（SIGSEGV段错误）问题。容器内安装的numpy 2.0.2与pandas 2.3.3存在兼容性问题，导致处理Excel文件时触发段错误。锁定numpy为1.26.4（稳定版本）和pandas为2.2.0（固定版本）以解决此问题。
...
**变更位置**：12-18
**变更内容**：
- 修改前：硬编码环境变量在environment部分
- 修改后：使用env_file配置加载.env.docker文件
- 移除硬编码的DATABASE_URL、APP_NAME、APP_VERSION
- 保留DEBUG=False和DEPLOY_MODE=unified

**变更原因**：配置生产环境使用.env.docker作为环境变量源，实现环境配置与代码分离。.env用于开发测试环境，.env.docker用于生产Docker环境。
...
**变更位置**：1-9
**变更内容**：
- 修改前：使用外部IP地址连接数据库 `10.23.65.95`
- 修改后：使用容器内部服务名连接数据库 `db:3306`
- 修复密码中的特殊字符编码问题
- 将DEBUG从True改为False
