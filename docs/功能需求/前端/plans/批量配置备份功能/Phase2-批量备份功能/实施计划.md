...
class TestBackupAllAPI:
    ...
    def test_get_backup_task_status(self):
        ...
        response = client.get("/configurations/backup-tasks/test-task-id")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_cancel_backup_task(self):
        ...
        response = client.post("/configurations/backup-tasks/test-task-id/cancel")
        assert response.status_code == 200
        data = response.json()
        assert data["message"] == "任务已取消"

    def test_backup_all_idempotency(self):
        """测试备份请求的幂等性保护"""
        client = TestClient(app)
        idempotency_key = "test-idempotency-key-12345"
        
        response1 = client.post(
            "/configurations/backup-all",
            json={"filter_status": None, "filter_vendor": None, "async_execute": True},
    ...
    if idempotency_key:
        ...
        if existing_task:
            return {
                ...
                "progress_percentage": existing_task.progress_percentage
            }
...
export default {
  name: 'BackupProgress',
  components: { Clock, Timer, Lightning },
  props: {
    title: { type: String, default: '批量备份进度' },
    ...
    startedAt: { type: [Date, String], default: null },
    completedAt: { type: [Date, String], default: null }
  },
  ...
  setup(props, { emit }) {
    ...
    const statusType = computed(() => {
      const types = {
        idle: 'info', pending: 'info', running: 'primary',
        completed: 'success', success: 'success', failed: 'danger',
        partial: 'warning', cancelled: 'info'
      }
    ...
    const statusText = computed(() => {
      const texts = {
        idle: '等待开始', pending: '等待执行', running: '进行中',
        completed: '已完成', success: '已完成', failed: '全部失败',
        partial: '部分失败', cancelled: '已取消'
      }
    ...
    const displayedErrors = computed(() => {
      if (showAllErrors.value) return props.errors
    ...
    const handleCancel = async () => {
      try {
        await ElMessageBox.confirm(
          ...
          '取消备份任务',
          { confirmButtonText: '确定取消', cancelButtonText: '继续备份', type: 'warning' }
        )
    ...
    return {
      cancelling, activeErrors, showAllErrors, progressPercentage, successRate,
      statusType, statusText, progressStatus, executionTime, displayedErrors,
      handleCancel, handleRetry, handleClose, formatTime, formatDuration
    }
  }
}
</script>
...
<style scoped>
.backup-progress { margin: 10px 0; }
.progress-card { width: 100%; }
.card-header { display: flex; justify-content: space-between; align-items: center; }
.header-left { display: flex; align-items: center; gap: 10px; }
.header-left .title { font-weight: bold; }
.task-id { font-size: 12px; color: #909399; }
.progress-content { padding: 10px 0; }
.progress-section { margin-bottom: 15px; }
.progress-stats { display: flex; justify-content: space-between; margin-bottom: 10px; padding: 10px; background: #f5f7fa; border-radius: 4px; }
...
.time-section { display: flex; gap: 20px; margin-bottom: 15px; color: #606266; }
...
.error-detail { padding: 10px; background: #fdf6ec; border-radius: 4px; }
.error-row { margin-bottom: 5px; }
.error-label { font-weight: bold; margin-right: 10px; }
.error-message { color: #f56c6c; }
.more-errors { text-align: center; padding: 10px; }
</style>
```
...
export default {
  components: { BackupProgress, BackupSettingsDialog },
  data() {
    return {
      backupSettingsVisible: false,
      backupProgress: {
        visible: false, total: 0, completed: 0, successCount: 0,
        failedCount: 0, status: 'idle', errors: [], taskId: null,
        startedAt: null, completedAt: null, pollingTimer: null
      }
    }
  },
  methods: {
    ...
    resetBackupProgress() {
      this.backupProgress = {
        visible: true, total: 0, completed: 0, successCount: 0,
        failedCount: 0, status: 'idle', errors: [], taskId: null,
        startedAt: null, completedAt: null, pollingTimer: null
      }
    },
      ...
      const doPoll = async () => {
        try {
          ...
          if (['completed', 'failed', 'cancelled'].includes(this.backupProgress.status)) {
            clearTimeout(this.backupProgress.pollingTimer)
...
class TestBackupIntegration:
    def test_full_backup_workflow(self, test_db, test_client):
        """测试完整的备份工作流程"""
        device_data = {
            "name": "Test-Switch", "ip": "192.168.100.1",
            "vendor": "Cisco", "username": "admin", "password": "secret", "status": "online"
        }
        ...
        backup_request = {
            "filter_status": None, "filter_vendor": None,
            "async_execute": True, "notify_on_complete": False,
            "max_concurrent": 3, "retry_count": 2
        }
    ...
    def test_cancel_backup_task(self, test_db, test_client):
        ...
        response = test_client.post("/configurations/backup-all", json={"async_execute": True})
        task_id = response.json()["task_id"]
    ...
    def test_backup_task_list(self, test_db, test_client):
        """测试获取备份任务列表"""
        for i in range(3):
            test_client.post("/configurations/backup-all", json={"async_execute": True})
...
git commit -m "test: 添加批量备份集成测试，覆盖任务取消和列表查询等场景"
```
...
- Phase 2评审: `docs/功能需求/前端/plans/批量配置备份功能/Phase2-批量备份功能/Phase2-批量备份功能-评审文档.md`
