...
from app.models.models import BackupExecutionLog, Device, BackupSchedule
from datetime import datetime
import uuid
import asyncio
import traceback
    ...
    async def _execute_backup(
        ...
        try:
            ...
            try:
                ...
                if result.get("config_size"):
                    log.config_size = result["config_size"]
                ...
                log.error_details = traceback.format_exc()
                db.commit()
...
async def get_execution_trend(
    ...
    trend_query = db.query(
        ...
        func.count(BackupExecutionLog.id).label('total'),
        func.sum(case((BackupExecutionLog.status == "success", 1), else_=0)).label('success'),
        func.sum(case((BackupExecutionLog.status == "failed", 1), else_=0)).label('failure'),
        func.avg(BackupExecutionLog.execution_time).label('avg_time')
    ).filter(
        BackupExecutionLog.created_at >= start_date
    ).group_by(
    ...
    while current_date <= end_date:
        ...
        success_counts.append(data['success'] or 0)
        failure_counts.append(data['failure'] or 0)
